...
const get_receipt_details = () => {
  ...
  return Promise.all(promises).then((promise_results) => {
    ...
    var transactions = _.reduce(answers.transactions.split(","), (acc, item) => {
    ...
    settings.transactions = answers.transactions;
...
const generate_receipt = (options) => {
  ...
  return new Promise((resolve, reject) => {
    pdf.create(template_file, PDF_SETTINGS).toFile(filename, (err, res) => {
      if (err) {
...
exports.run = (should_edit) => {
  ...
  settings.getSettings().then((receipt_settings) => {
    if (should_edit) {
      ...
      edit_settings(receipt_settings).then(settings.createSettingsFile).then(() => {
